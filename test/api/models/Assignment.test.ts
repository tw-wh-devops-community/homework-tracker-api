...
describe('Assignment model test', () => {
  ...
  it('should get the saved assignment info', async () => {
    ...
    const queryInterviewer = await Interviewer.findOne({ _id: latestAssignment.interviewer_id }).exec()
    const queryHomework = await Homework.findOne({ _id: latestAssignment.homework_id }).exec()
